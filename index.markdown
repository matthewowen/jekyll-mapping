...
* Set multiple locations on a single piece of content and map them all (perfect if you want a map showing all of your company's offices): [see an example]({{site.baseurl}}multiple-locations)
* Set a KML layer (or multiple KML layers) on a piece of content and display on a map. This one offers a world of possibilities: data overlays, directions or routes, up to the minute data from a continually updating feed: [see an example]({{site.baseurl}}kml-layer)
